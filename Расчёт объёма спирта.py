...
def koef(event):
    ...
    #Описание формулы, мы берём объём акоголя в МЛ, делим объём на 100%, узнаём столько МЛ в одном проценте.
    EntryD.delete(0, END) # очищаем текстовое поле полностью
    EntryD.insert(0, result) # вставляем результат в начало 

#Данная формула используется для расчета количества воды, которое необходимо добавить к алкогольному напитку, чтобы разбавить его до желаемой концентрации.
